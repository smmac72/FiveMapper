...
// vertex and cb layouts (must match hlsl)
...
struct LightCBShadow { XMFLOAT4X4 lightVP; };

// helper: create upload buffer
...
// simple high-res timer for dt
struct HiTimer
{
    LARGE_INTEGER freq{}, last{};
    void init(){ QueryPerformanceFrequency(&freq); QueryPerformanceCounter(&last); }
    float tick(){ LARGE_INTEGER now; QueryPerformanceCounter(&now);
        double d = double(now.QuadPart - last.QuadPart) / double(freq.QuadPart);
        last = now; return float(d); }
};
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE, PWSTR, int)
{
    ...
    ComPtr<ID3D12GraphicsCommandList> cmdList;
    {
        ...
        if (FAILED(devRes.GetDevice()->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT,
    ...
    while (window.processMessages())
    {
        ...
        if (listOpen) { cmdList->Close(); listOpen = false; }
        cmdAlloc[fi]->Reset();
        ...
        if (window.isActive() && window.isMouseCaptured() && (md.x || md.y))
        {
        ...
        if (window.isActive())
        {
            ...
            XMFLOAT3 dMove{}; XMStoreFloat3(&dMove, delta);
            camPos.x += dMove.x; camPos.y += dMove.y; camPos.z += dMove.z;
        }
        ...
        CameraCBLight camL{};
        XMStoreFloat4x4(&camL.invProj, invProj);
        XMStoreFloat4x4(&camL.invView, invView);
        ...
        {
            D3D12_CPU_DESCRIPTOR_HANDLE mrt[4] = { gbuf.GetRTV(0), gbuf.GetRTV(1), gbuf.GetRTV(2), gbuf.GetRTV(3) };
