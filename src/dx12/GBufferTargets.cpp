...
static ComPtr<ID3D12Resource> CreateTex2D(
    ...
    const D3D12_CLEAR_VALUE* optClear // pass clear for optimized clears
)
{
    ...
    if (FAILED(dev->CreateCommittedResource(
        &heap, D3D12_HEAP_FLAG_NONE, &tex, initialState, optClear, IID_PPV_ARGS(&res))))
    {
...
{
    // we pass clear values so clears are potentially faster
    D3D12_CLEAR_VALUE cv0{}; cv0.Format = fmts.g0; cv0.Color[0] = 0; cv0.Color[1] = 0; cv0.Color[2] = 0; cv0.Color[3] = 1;
    D3D12_CLEAR_VALUE cv1{}; cv1.Format = fmts.g1; cv1.Color[0] = 0; cv1.Color[1] = 0; cv1.Color[2] = 0; cv1.Color[3] = 1;
    D3D12_CLEAR_VALUE cv2{}; cv2.Format = fmts.g2; cv2.Color[0] = 0; cv2.Color[1] = 0; cv2.Color[2] = 0; cv2.Color[3] = 1;
    D3D12_CLEAR_VALUE cv3{}; cv3.Format = fmts.g3; cv3.Color[0] = 0; cv3.Color[1] = 0; cv3.Color[2] = 0; cv3.Color[3] = 1;

    m_gbuf[0] = CreateTex2D(device, fmts.g0, m_width, m_height,
                            D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET,
                            D3D12_RESOURCE_STATE_COMMON, &cv0);
    m_gbuf[1] = CreateTex2D(device, fmts.g1, m_width, m_height,
                            D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET,
                            D3D12_RESOURCE_STATE_COMMON, &cv1);
    m_gbuf[2] = CreateTex2D(device, fmts.g2, m_width, m_height,
                            D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET,
                            D3D12_RESOURCE_STATE_COMMON, &cv2);
    m_gbuf[3] = CreateTex2D(device, fmts.g3, m_width, m_height,
                            D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET,
                            D3D12_RESOURCE_STATE_COMMON, &cv3);
    ...
    m_depth = CreateTex2D(device, DXGI_FORMAT_R32_TYPELESS, m_width, m_height,
                          ...
                          D3D12_RESOURCE_STATE_COMMON, &dcv);
}
...
{
    // 6 descriptors: g0..g3, depth, shadow (to be filled)
    D3D12_DESCRIPTOR_HEAP_DESC desc{};
    ...
    // slot 5 (shadow) left empty, will be created by ShadowMap::CreateSRV(...)
}
