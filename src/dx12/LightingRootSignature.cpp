...
{
    // b0: camera (inv proj + inv view + camera position)
    // b1: sun/light params
    ...
    // static samplers:
    // s0: linear clamp (generic)
    // s1: point clamp (g-buffer/depth reads)
    // s2: comparison linear clamp lequal (shadow)
    ...
    ss[2].ShaderRegister = 2;
    ss[2].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;
    ss[2].BorderColor = D3D12_STATIC_BORDER_COLOR_OPAQUE_WHITE;
